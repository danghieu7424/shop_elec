...
export default function ProductDetail() {
    ...
    // --- LOGIC SỐ LƯỢNG (NÂNG CẤP) ---
    const handleQuantityChange = (delta) => {
        const newQty = quantity + delta;
        if (newQty >= 1 && newQty <= product.stock) setQuantity(newQty);
    };

    // Xử lý khi nhập tay vào ô input
    const handleQuantityInput = (e) => {
        const val = parseInt(e.target.value);
        if (!isNaN(val)) setQuantity(val); // Cho phép nhập số tạm thời
        else setQuantity(''); // Xóa trắng nếu nhập bậy
    };

    // Xử lý khi focus out (blur) để kiểm tra tính hợp lệ
    const handleQuantityBlur = () => {
        let finalQty = quantity;
        if (!finalQty || finalQty < 1) finalQty = 1;
        if (finalQty > product.stock) {
            finalQty = product.stock;
            alert(`Xin lỗi, chúng tôi chỉ còn ${product.stock} sản phẩm trong kho.`);
        }
    ...
    const handleAddToCart = async () => {
        ...
        // 2. Nếu đã đăng nhập -> GỌI API LƯU VÀO DB
        if (userInfo) {
            // Tính toán số lượng mới: (Số lượng đã có trong giỏ) + (Số lượng vừa thêm)
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid md:grid-cols-12 gap-8 mb-12">
                    ...
                    {productImages.length > 1 && (
                        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-gray-300">
                            {productImages.map((img, idx) => (
                                ...
                                >
                                    <img src={`${domain}${img}`} className="w-full h-full object-contain" alt={`thumb-${idx}`}/>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div className="md:col-span-7 space-y-6">
                    ...
                    {/* Bộ chọn số lượng & Nút mua (SỬA ĐOẠN NÀY) */}
                    <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t">
                        <div className="flex items-center border rounded-lg w-fit bg-white">
                            <button onClick={() => handleQuantityChange(-1)} className="p-3 hover:bg-gray-100 disabled:opacity-50 border-r" disabled={quantity <= 1}><Minus size={18}/></button>
                            
                            {/* Input Nhập Tay */}
                            <input 
                                className="w-20 text-center font-bold outline-none bg-transparent py-3 text-gray-700" 
                                value={quantity} 
                                onChange={handleQuantityInput} // Cho phép nhập
                                onBlur={handleQuantityBlur}   // Kiểm tra khi nhập xong
                                type="number"
                            />
                            
                            <button onClick={() => handleQuantityChange(1)} className="p-3 hover:bg-gray-100 disabled:opacity-50 border-l" disabled={quantity >= product.stock}><Plus size={18}/></button>
                        </div>
            ...
            {/* --- KHU VỰC TABS (Giữ nguyên code Tabs) --- */}
            <div className="bg-white rounded-xl border shadow-sm overflow-hidden mt-12">
                <div className="flex border-b overflow-x-auto">
                    {['desc', 'specs', 'reviews'].map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-4 font-bold border-b-2 whitespace-nowrap transition-colors capitalize ${activeTab === tab ? 'border-blue-600 text-blue-600 bg-blue-50' : 'border-transparent text-gray-600 hover:bg-gray-50'}`}>
                ...
                <div className="p-6 md:p-8 min-h-[300px]">
                    {activeTab === 'desc' && <div className="prose max-w-none text-gray-700 whitespace-pre-line leading-relaxed">{product.description || "Chưa có mô tả."}</div>}
                    {activeTab === 'specs' && (
                        <div>
                            {specsArray.length > 0 ? (
                                <table className="w-full max-w-2xl text-sm border-collapse"><tbody>{specsArray.map((spec, index) => (<tr key={index} className="border-b last:border-0"><td className="py-3 px-4 bg-gray-50 font-medium w-1/3 text-gray-600 capitalize">{spec.label}</td><td className="py-3 px-4 text-gray-800">{spec.value}</td></tr>))}</tbody></table>
                            ...
                            <div className="bg-gray-50 p-6 rounded-lg mb-8 border shadow-sm">
                                ...
                                {userInfo ? (
                                    <div className="space-y-4">
                                        ...
                                        <textarea className="w-full border rounded-lg p-3 focus:ring-2 ring-blue-500 outline-none resize-none bg-white" rows="3" placeholder="Chia sẻ cảm nhận..." value={newComment} onChange={e => setNewComment(e.target.value)}/>
                                        <div className="flex justify-end"><Button onClick={handlePostReview} disabled={submitting}>{submitting ? "Đang gửi..." : "Gửi đánh giá"}</Button></div>
                                    </div>
                                ) : (<div className="text-gray-500 bg-white p-4 rounded border text-center">Vui lòng <span className="text-blue-600 font-bold cursor-pointer hover:underline" onClick={() => navigate('/login')}>đăng nhập</span> để viết đánh giá.</div>)}
                            </div>
                            <div className="space-y-6">
                                {reviews.length === 0 ? (<div className="text-center text-gray-500 py-10 bg-gray-50 rounded-lg border border-dashed">Chưa có đánh giá nào.</div>) : (reviews.map((review) => (
                                    <div key={review.id} className="border-b pb-6 last:border-0 last:pb-0">
                                        <div className="flex items-center gap-3 mb-2">
                                            <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-bold text-lg select-none">{review.user_name ? review.user_name.charAt(0).toUpperCase() : <User size={20}/>}</div>
